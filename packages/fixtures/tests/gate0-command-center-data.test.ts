...
describe("Gate 0 command center surface", () => {
  it("keeps the command center tied to Gate 2 simulation-planning scope", () => {
    ...
    expect(data).toContain('latestPacket: "TRD-756"');
  });
  ...
  it("checkpoints the evidence-control lane without opening new scope", () => {
    ...
    expect(data).toContain("TRD-756 accepts the bounded market-intelligence foundation.");
    expect(data).toContain("TRD-757 is authorized for one local read-only Intelligence Brief MVP.");
  });
  ...
  it("records market intelligence truth without adding prediction or execution authority", () => {
    ...
    expect(data).toContain("TRD-756 closes the intelligence foundation checkpoint.");
