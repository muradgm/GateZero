...
describe("Gate 2 manual review authoring UI", () => {
  ...
  it("fails closed for stale or malformed stored records", () => {
    ...
    expect(mainSource).toContain("const isValidLocalRecord = (record) =>");
    expect(mainSource).toContain("record.brief_content_sha256 === brief.contentHash");
  });

  it("renders read-only local review history without action controls", () => {
    expect(mainSource).toContain('id="manual-review-history"');
    expect(mainSource).toContain("traderframe.manual-review-history.v1");
    expect(mainSource).toContain("Authority");
    expect(mainSource).toContain("none granted");
    expect(mainSource).not.toContain("restore review");
    expect(mainSource).not.toContain("promote review");
  });
});
