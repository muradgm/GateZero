  ...
  return (
    <Panel className="confidence-heatmap-panel">
      <PanelHeading
        ...
        aside={<Badge tone={totals.net >= 0 ? "success" : "warning"}>{candidate.report.confidence}</Badge>}
      />
      ...
      <div className="confidence-heatmap-summary">
        ...
        <div>
          ...
          <strong>{totals.net > 0 ? "+" : ""}{totals.net}</strong>
        </div>
      </div>

      <div className="confidence-heatmap-grid">
        {dimensions.map((dimension) => (
          <details className={`confidence-dimension confidence-dimension--${dimension.tone}`} key={dimension.key}>
            <summary>
              <div>
                <span>{dimension.label}</span>
                <small>{dimension.status}</small>
              </div>
              <strong>{dimension.points > 0 ? "+" : ""}{dimension.points}</strong>
            </summary>
            <div className="confidence-track" aria-label={`${dimension.label} confidence ${dimension.strength}%`}>
              <span style={{ width: `${dimension.strength}%` }} />
            </div>
            <div className="confidence-dimension__detail">
              {dimension.reasons.map((reason) => <p key={reason}>{reason}</p>)}
              {dimension.limitations.map((limitation) => <small key={limitation}>{limitation}</small>)}
            </div>
          </details>
        ))}
      </div>
      ...
      <p className="confidence-heatmap-note">
        Confidence summarizes evidence quality and agreement. It is not a probability of profit and does not override risk review.
      </p>
    </Panel>
  );
}
...
function buildDimensions(contributions) {
  ...
  return [...grouped.values()]
    .map((dimension) => {
      ...
      const tone = dimension.points > 0 ? "positive" : dimension.points < 0 ? "negative" : "neutral";
      const status = dimension.points >= 12 ? "Strong support" : dimension.points > 0 ? "Moderate support" : dimension.points <= -12 ? "Strong contradiction" : dimension.points < 0 ? "Moderate contradiction" : "Neutral";
      return {
        ...dimension,
...
function summarizeDimensions(dimensions) {
  const supporting = dimensions.filter((item) => item.points > 0).reduce((sum, item) => sum + item.points, 0);
  const contradicting = Math.abs(dimensions.filter((item) => item.points < 0).reduce((sum, item) => sum + item.points, 0));
